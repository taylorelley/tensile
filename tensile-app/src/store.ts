...
import { BUILTIN_EXERCISES } from './exerciseCatalog';
...
export interface UserProfile {
  ...
  /** Multi-block periodisation plan generated by planProgram() when meetDate is set.
   *  Consumed by generateNextDevelopmentBlock() to select the next phase. */
  programPlan?: BlockPlan[];
...
export function generateBlock(profile: UserProfile, phase: BlockPhase = 'ACCUMULATION'): Block {
  ...
  const dayPlans = planBlock(profile, phase, monday, weeks, BUILTIN_EXERCISES);

  const sessions: Session[] = dayPlans.map(({ dayIndex, weekIndex, date, exercises, durationTrimmed }) => ({
    id: `sess-${blockId}-w${weekIndex}-${dayIndex}`,
    blockId,
    scheduledDate: date,
  ...
  persist(
    (set, get) => ({
      ...
      generateNextDevelopmentBlock: () => {
        ...
        // Regenerate the program plan if meetDate is set and no plan exists yet.
        const updatedProgramPlan: BlockPlan[] | undefined =
          profile.meetDate ? (profile.programPlan ?? planProgram(profile)) : undefined;
        // Phase from program plan if available, otherwise cycle through phases.
        const nextPhase: BlockPhase = nextMode === 'TTP'
          ? 'ACCUMULATION'
          : (updatedProgramPlan?.[completedCount]?.phase ?? phaseOrder[completedCount % 3]);
